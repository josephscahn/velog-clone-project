  ...
  async selectCommentList(post_id: number, login_user_id: number) {
    const comments = this.query(
      ...
      SELECT
        ...
        JSON_ARRAYAGG(
          JSON_OBJECT(
            ...
            'comment_profile_image', IF(user.profile_image=null, null, CONCAT('${process.env.IMAGE_URL}', user.profile_image)),
            'comment_id', child.id,
      ...
      SELECT 
        ...
        IF(user.profile_image=null, null, CONCAT('${process.env.IMAGE_URL}', user.profile_image)) AS comment_profile_image,
